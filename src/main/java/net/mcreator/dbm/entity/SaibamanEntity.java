...
import net.mcreator.dbm.procedures.SaibamanDefeatedProcedure;
import net.mcreator.dbm.procedures.SaibamanAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class SaibamanEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(SaibamanEntity.class, EntityDataSerializers.BOOLEAN);

	public SaibamanEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.SAIBAMAN.get(), world);
	}

	public SaibamanEntity(EntityType<SaibamanEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = SaibamanEntity.this.getRandom();
				double dir_x = SaibamanEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = SaibamanEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = SaibamanEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = SaibamanEntity.this.getX();
				double y = SaibamanEntity.this.getY();
				double z = SaibamanEntity.this.getZ();
				Entity entity = SaibamanEntity.this;
				Level world = SaibamanEntity.this.level();
	...
	public void baseTick() {
		super.baseTick();
		SaibamanAttackPatternProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}
