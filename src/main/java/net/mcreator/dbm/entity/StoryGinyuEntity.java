...
import net.mcreator.dbm.procedures.GinyuDefeatedProcedure;
import net.mcreator.dbm.procedures.BasicAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryGinyuEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryGinyuEntity.class, EntityDataSerializers.STRING);

	public StoryGinyuEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_GINYU.get(), world);
	}

	public StoryGinyuEntity(EntityType<StoryGinyuEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryGinyuEntity.this.getRandom();
				double dir_x = StoryGinyuEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryGinyuEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryGinyuEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryGinyuEntity.this.getX();
				double y = StoryGinyuEntity.this.getY();
				double z = StoryGinyuEntity.this.getZ();
				Entity entity = StoryGinyuEntity.this;
				Level world = StoryGinyuEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		GinyuDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
