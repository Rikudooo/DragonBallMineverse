...
import net.mcreator.dbm.procedures.GuldoDefeatedProcedure;
import net.mcreator.dbm.procedures.BasicAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryGuldoEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryGuldoEntity.class, EntityDataSerializers.STRING);

	public StoryGuldoEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_GULDO.get(), world);
	}

	public StoryGuldoEntity(EntityType<StoryGuldoEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryGuldoEntity.this.getRandom();
				double dir_x = StoryGuldoEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryGuldoEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryGuldoEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryGuldoEntity.this.getX();
				double y = StoryGuldoEntity.this.getY();
				double z = StoryGuldoEntity.this.getZ();
				Entity entity = StoryGuldoEntity.this;
				Level world = StoryGuldoEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		GuldoDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
	...
	public static AttributeSupplier.Builder createAttributes() {
		...
		builder = builder.add(Attributes.ATTACK_DAMAGE, 190);
