...
import net.mcreator.dbm.procedures.JeiceDefeatedProcedure;
import net.mcreator.dbm.procedures.BasicAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryJeiceEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryJeiceEntity.class, EntityDataSerializers.STRING);

	public StoryJeiceEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_JEICE.get(), world);
	}

	public StoryJeiceEntity(EntityType<StoryJeiceEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryJeiceEntity.this.getRandom();
				double dir_x = StoryJeiceEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryJeiceEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryJeiceEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryJeiceEntity.this.getX();
				double y = StoryJeiceEntity.this.getY();
				double z = StoryJeiceEntity.this.getZ();
				Entity entity = StoryJeiceEntity.this;
				Level world = StoryJeiceEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		JeiceDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
