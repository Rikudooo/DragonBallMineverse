...
import net.mcreator.dbm.procedures.CuiDefeatedProcedure;
import net.mcreator.dbm.procedures.BasicAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryCuiEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryCuiEntity.class, EntityDataSerializers.STRING);

	public StoryCuiEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_CUI.get(), world);
	}

	public StoryCuiEntity(EntityType<StoryCuiEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryCuiEntity.this.getRandom();
				double dir_x = StoryCuiEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryCuiEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryCuiEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryCuiEntity.this.getX();
				double y = StoryCuiEntity.this.getY();
				double z = StoryCuiEntity.this.getZ();
				Entity entity = StoryCuiEntity.this;
				Level world = StoryCuiEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		CuiDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
