...
import net.mcreator.dbm.procedures.ShisamiDefeatedProcedure;
import net.mcreator.dbm.procedures.BasicAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryShisamiEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryShisamiEntity.class, EntityDataSerializers.STRING);

	public StoryShisamiEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_SHISAMI.get(), world);
	}

	public StoryShisamiEntity(EntityType<StoryShisamiEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryShisamiEntity.this.getRandom();
				double dir_x = StoryShisamiEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryShisamiEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryShisamiEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryShisamiEntity.this.getX();
				double y = StoryShisamiEntity.this.getY();
				double z = StoryShisamiEntity.this.getZ();
				Entity entity = StoryShisamiEntity.this;
				Level world = StoryShisamiEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		ShisamiDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
	...
	public static AttributeSupplier.Builder createAttributes() {
		...
		builder = builder.add(Attributes.ATTACK_DAMAGE, 1500);
