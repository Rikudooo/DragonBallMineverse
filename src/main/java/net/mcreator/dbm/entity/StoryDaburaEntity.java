...
import net.mcreator.dbm.procedures.DaburaDefeatedProcedure;
import net.mcreator.dbm.procedures.BasicAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryDaburaEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryDaburaEntity.class, EntityDataSerializers.STRING);

	public StoryDaburaEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_DABURA.get(), world);
	}

	public StoryDaburaEntity(EntityType<StoryDaburaEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryDaburaEntity.this.getRandom();
				double dir_x = StoryDaburaEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryDaburaEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryDaburaEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryDaburaEntity.this.getX();
				double y = StoryDaburaEntity.this.getY();
				double z = StoryDaburaEntity.this.getZ();
				Entity entity = StoryDaburaEntity.this;
				Level world = StoryDaburaEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		DaburaDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
	...
	public static AttributeSupplier.Builder createAttributes() {
		...
		builder = builder.add(Attributes.ATTACK_DAMAGE, 1700);
