...
import net.mcreator.dbm.procedures.KibitoDefeatedProcedure;
import net.mcreator.dbm.procedures.BasicAttackPatternProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryKibitoEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryKibitoEntity.class, EntityDataSerializers.STRING);

	public StoryKibitoEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_KIBITO.get(), world);
	}

	public StoryKibitoEntity(EntityType<StoryKibitoEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryKibitoEntity.this.getRandom();
				double dir_x = StoryKibitoEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryKibitoEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryKibitoEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryKibitoEntity.this.getX();
				double y = StoryKibitoEntity.this.getY();
				double z = StoryKibitoEntity.this.getZ();
				Entity entity = StoryKibitoEntity.this;
				Level world = StoryKibitoEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		KibitoDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
