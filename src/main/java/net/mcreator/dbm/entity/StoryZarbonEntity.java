...
import net.mcreator.dbm.procedures.ZarbonDefeatedProcedure;
import net.mcreator.dbm.procedures.StoryMobFlyConditionProcedure;
import net.mcreator.dbm.init.DbmModEntities;

public class StoryZarbonEntity extends Monster {
	public static final EntityDataAccessor<String> DATA_SelectedKiAttack = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_KiAttackSize = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldYAW = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_OldPITCH = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_KiAttackHits = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Scale = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Integer> DATA_Ki = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<Boolean> DATA_KiCharging = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> DATA_Form = SynchedEntityData.defineId(StoryZarbonEntity.class, EntityDataSerializers.STRING);

	public StoryZarbonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(DbmModEntities.STORY_ZARBON.get(), world);
	}

	public StoryZarbonEntity(EntityType<StoryZarbonEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1.2, 20) {
			...
			protected Vec3 getPosition() {
				RandomSource random = StoryZarbonEntity.this.getRandom();
				double dir_x = StoryZarbonEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = StoryZarbonEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = StoryZarbonEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
				return new Vec3(dir_x, dir_y, dir_z);
			}

			@Override
			public boolean canUse() {
				double x = StoryZarbonEntity.this.getX();
				double y = StoryZarbonEntity.this.getY();
				double z = StoryZarbonEntity.this.getZ();
				Entity entity = StoryZarbonEntity.this;
				Level world = StoryZarbonEntity.this.level();
	...
	public void die(DamageSource source) {
		super.die(source);
		ZarbonDefeatedProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
	...
	public static AttributeSupplier.Builder createAttributes() {
		...
		builder = builder.add(Attributes.ATTACK_DAMAGE, 160);
