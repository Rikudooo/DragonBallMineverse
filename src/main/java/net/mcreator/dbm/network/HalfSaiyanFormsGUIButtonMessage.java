...
public class HalfSaiyanFormsGUIButtonMessage {
	private final int buttonID, x, y, z;

	public HalfSaiyanFormsGUIButtonMessage(FriendlyByteBuf buffer) {
	...
	public HalfSaiyanFormsGUIButtonMessage(int buttonID, int x, int y, int z) {
	...
	public static void buffer(HalfSaiyanFormsGUIButtonMessage message, FriendlyByteBuf buffer) {
	...
	public static void handler(HalfSaiyanFormsGUIButtonMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = HalfSaiyanFormsGUIMenu.guistate;
		...
		if (buttonID == 15) {
			...
			SetFormPathSSJ2Procedure.execute(entity);
		}
		if (buttonID == 16) {
		...
		if (buttonID == 17) {
			...
			SetFormPathSSJProcedure.execute(entity);
		}
		if (buttonID == 18) {
		...
		if (buttonID == 19) {
			...
			SetFormMysticProcedure.execute(entity);
		}
		if (buttonID == 20) {

			SetFormSSJRageProcedure.execute(entity);
		}
		if (buttonID == 21) {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		DbmMod.addNetworkMessage(HalfSaiyanFormsGUIButtonMessage.class, HalfSaiyanFormsGUIButtonMessage::buffer, HalfSaiyanFormsGUIButtonMessage::new, HalfSaiyanFormsGUIButtonMessage::handler);
	}
}
