...
public class RenderCloudsProcedure {
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("dbm:namek"))) {
			RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "cloudsnamek" + ".png")));
			renderClouds(Minecraft.getInstance().options.getCloudsType(), 192, -1, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255);
		}
	}
}
