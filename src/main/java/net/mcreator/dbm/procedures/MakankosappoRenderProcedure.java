...
public class MakankosappoRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (MakankosappoRenderProcedure.bufferBuilder == null || !MakankosappoRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (MakankosappoRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					MakankosappoRenderProcedure.mode = mode;
					MakankosappoRenderProcedure.format = format;
					MakankosappoRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					MakankosappoRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					MakankosappoRenderProcedure.mode = mode;
					MakankosappoRenderProcedure.format = format;
					MakankosappoRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					MakankosappoRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		MakankosappoRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			MakankosappoRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, double partialTick, double ticks) {
		...
		for (Entity entityiterator : new ArrayList<>(world.players())) {
			if ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).LaunchKiAttack == true) {
				if (((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).SelectedKiAttack).equals("Makankosappo")) {
					if (target(2)) {
						speed = 0.4;
						width = 0.5;
						...
						RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "kiattacks/makankosappo_wave" + ".png")));
					...
					if (target(2)) {
						...
						width = 0.8;
						length = (entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiAttackSize;
						RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "kiattacks/makankosappo_spiral" + ".png")));
						if (begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX_COLOR, (!Minecraft.getInstance().isPaused()))) {
							i = width / (-2);
							j = width / 2;
							k = length - 0.5;
