...
public class KiSenseKiProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (KiSenseKiProcedure.bufferBuilder == null || !KiSenseKiProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (KiSenseKiProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					KiSenseKiProcedure.mode = mode;
					KiSenseKiProcedure.format = format;
					KiSenseKiProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KiSenseKiProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					KiSenseKiProcedure.mode = mode;
					KiSenseKiProcedure.format = format;
					KiSenseKiProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KiSenseKiProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		KiSenseKiProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			KiSenseKiProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, double partialTick) {
		...
		if (!((entity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiSenseTarget).equals("Off")
				&& (entity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiSenseLevel >= 1) {
					...
					for (Entity entityiterator : _entfound) {
						...
						if (!(TargetedEntity == entity) && !(TargetedEntity instanceof Player || TargetedEntity instanceof ServerPlayer) && TargetedEntity instanceof LivingEntity) {
							if (begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR, true)) {
								...
								add(0, 0.75, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255);
							...
							if (target(2)) {
								...
								renderShape(shape(), (TargetedEntity.getX()), (TargetedEntity.getY() + TargetedEntity.getBbHeight() + 0.3), (TargetedEntity.getZ()), entity.getViewYRot((float) partialTick), entity.getViewXRot((float) partialTick), 0,
										1, (float) ((TargetedEntity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).Ki
												/ (TargetedEntity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).MaxKi),
										1, 255 << 24 | 16 << 16 | 167 << 8 | 242);
								release();
							}
						}
					}
