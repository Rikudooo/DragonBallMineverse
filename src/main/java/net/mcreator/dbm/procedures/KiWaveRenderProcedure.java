...
public class KiWaveRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (KiWaveRenderProcedure.bufferBuilder == null || !KiWaveRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (KiWaveRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					KiWaveRenderProcedure.mode = mode;
					KiWaveRenderProcedure.format = format;
					KiWaveRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KiWaveRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					KiWaveRenderProcedure.mode = mode;
					KiWaveRenderProcedure.format = format;
					KiWaveRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KiWaveRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		KiWaveRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			KiWaveRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
