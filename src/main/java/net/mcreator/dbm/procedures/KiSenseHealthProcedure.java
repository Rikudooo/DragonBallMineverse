...
public class KiSenseHealthProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (KiSenseHealthProcedure.bufferBuilder == null || !KiSenseHealthProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (KiSenseHealthProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					KiSenseHealthProcedure.mode = mode;
					KiSenseHealthProcedure.format = format;
					KiSenseHealthProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KiSenseHealthProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					KiSenseHealthProcedure.mode = mode;
					KiSenseHealthProcedure.format = format;
					KiSenseHealthProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KiSenseHealthProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		KiSenseHealthProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			KiSenseHealthProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, double partialTick) {
		...
		if (!((entity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiSenseTarget).equals("Off")
				&& (entity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiSenseLevel >= 2) {
						...
						if (!(TargetedEntity == entity) && !(TargetedEntity instanceof Player || TargetedEntity instanceof ServerPlayer) && TargetedEntity instanceof LivingEntity) {
							...
							if (target(2)) {
								renderShape(shape(), (TargetedEntity.getX()), (TargetedEntity.getY() + TargetedEntity.getBbHeight() + 0.3), (TargetedEntity.getZ()), entity.getViewYRot((float) partialTick), entity.getViewXRot((float) partialTick), 0,
										1, (float) ((TargetedEntity instanceof LivingEntity _livEnt ? _livEnt.getHealth() : -1) / (TargetedEntity instanceof LivingEntity _livEnt ? _livEnt.getMaxHealth() : -1)), 1, 255 << 24 | 0 << 16 | 175 << 8 | 0);
								release();
							}
						}
					}
