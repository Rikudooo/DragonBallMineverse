...
public class DoubleSundayChargingRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (DoubleSundayChargingRenderProcedure.bufferBuilder == null || !DoubleSundayChargingRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (DoubleSundayChargingRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					DoubleSundayChargingRenderProcedure.mode = mode;
					DoubleSundayChargingRenderProcedure.format = format;
					DoubleSundayChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					DoubleSundayChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					DoubleSundayChargingRenderProcedure.mode = mode;
					DoubleSundayChargingRenderProcedure.format = format;
					DoubleSundayChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					DoubleSundayChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		DoubleSundayChargingRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			DoubleSundayChargingRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		...
		double xcos = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				if ((entityiterator instanceof StoryRaditzEntity _datEntS ? _datEntS.getEntityData().get(StoryRaditzEntity.DATA_SelectedKiAttack) : "").equals("Double Sunday")) {
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX() - Math.cos(Math.toRadians(entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0)) * 0.5),
								(entityiterator.getY() + entityiterator.getEyeHeight() * 0.6),
								(entityiterator.getZ() - Math.sin(Math.toRadians(entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0)) * 0.5),
								(float) (entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0),
								(float) (entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0), 0, (float) 1.5, (float) 1.5, (float) 1.5, 255 << 24 | 236 << 16 | 194 << 8 | 185);
						renderShape(shape(), (entityiterator.getX() + Math.cos(Math.toRadians(entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0)) * 0.5),
								(entityiterator.getY() + entityiterator.getEyeHeight() * 0.6),
								(entityiterator.getZ() + Math.sin(Math.toRadians(entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0)) * 0.5),
								(float) (entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0),
								(float) (entityiterator instanceof StoryRaditzEntity _datEntI ? _datEntI.getEntityData().get(StoryRaditzEntity.DATA_OldPITCH) : 0), 0, (float) 1.5, (float) 1.5, (float) 1.5, 255 << 24 | 236 << 16 | 194 << 8 | 187);
						release();
					}
				}
			}
