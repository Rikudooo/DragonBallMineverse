...
public class SkyRenderProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != SkyRenderProcedure.amount || seed != SkyRenderProcedure.seed) {
			SkyRenderProcedure.amount = amount;
			SkyRenderProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	private static boolean execute(@Nullable Event event, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("dbm:space"))) {
			...
			renderStars(15000, 10842, 90, 0, 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, false);
		}
