...
public class SkyRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (SkyRenderProcedure.bufferBuilder == null || !SkyRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (SkyRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					SkyRenderProcedure.mode = mode;
					SkyRenderProcedure.format = format;
					SkyRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					SkyRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					SkyRenderProcedure.mode = mode;
					SkyRenderProcedure.format = format;
					SkyRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					SkyRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		SkyRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			SkyRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("dbm:hyperbolic_time_chamber"))) {
			if (target(1)) {
				system(false);
				size = Minecraft.getInstance().gameRenderer.getRenderDistance() * 2;
				RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "whitesky" + ".png")));
				renderShape(shape(), 0, 0, 0, 0, 0, 0, (float) size, (float) size, (float) size, 255 << 24 | 255 << 16 | 255 << 8 | 255);
				release();
			}
			if (begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX_COLOR, false)) {
