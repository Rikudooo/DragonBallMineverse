...
public class KamehamehaRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (KamehamehaRenderProcedure.bufferBuilder == null || !KamehamehaRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (KamehamehaRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					KamehamehaRenderProcedure.mode = mode;
					KamehamehaRenderProcedure.format = format;
					KamehamehaRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KamehamehaRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					KamehamehaRenderProcedure.mode = mode;
					KamehamehaRenderProcedure.format = format;
					KamehamehaRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KamehamehaRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		KamehamehaRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			KamehamehaRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
		...
		for (Entity entityiterator : new ArrayList<>(world.players())) {
			if ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).LaunchKiAttack == true) {
				if (((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).SelectedKiAttack).equals("Kamehameha")) {
					x = Math.sin(Math.toRadians((entity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).OldYAW)) * 1.5;
					z = Math.cos(Math.toRadians((entity.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).OldYAW)) * 1.5;
					if (target(2)) {
						speed = 0.4;
						width = 1;
						...
						RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "kiattacks/kamehameha_wave" + ".png")));
