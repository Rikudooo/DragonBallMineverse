...
public class FormTimerOverlayProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			FormTimerOverlayProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				FormTimerOverlayProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (target(2)) {
			...
			var1 = (player.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).TransformationTimer;
			renderRectangle(30, 12, (float) (30 + 83 * (var1 / 100)), 14, 0, 255 << 24 | 60 << 16 | 50 << 8 | 255);
			release();
		}
	}
}
