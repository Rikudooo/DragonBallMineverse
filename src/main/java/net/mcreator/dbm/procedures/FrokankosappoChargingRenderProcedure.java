...
public class FrokankosappoChargingRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (FrokankosappoChargingRenderProcedure.bufferBuilder == null || !FrokankosappoChargingRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (FrokankosappoChargingRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					FrokankosappoChargingRenderProcedure.mode = mode;
					FrokankosappoChargingRenderProcedure.format = format;
					FrokankosappoChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					FrokankosappoChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					FrokankosappoChargingRenderProcedure.mode = mode;
					FrokankosappoChargingRenderProcedure.format = format;
					FrokankosappoChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					FrokankosappoChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		FrokankosappoChargingRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			FrokankosappoChargingRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
		...
		for (Entity entityiterator : new ArrayList<>(world.players())) {
			if ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).LaunchKiAttack == true) {
				if (((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).SelectedKiAttack).equals("Frokankusappo")) {
					x = Math.sin(Math.toRadians((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).OldYAW)) * (-1.1);
					z = Math.cos(Math.toRadians((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).OldYAW)) * 1.1;
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX() + x), (entityiterator.getY() + entityiterator.getEyeHeight() * 0.8), (entityiterator.getZ() + z),
								...
								(float) (entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).OldPitch, 0, (float) 0.46, (float) 0.46, (float) 0.46,
								255 << 24 | 51 << 16 | 175 << 8 | 255);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX() + x), (entityiterator.getY() + entityiterator.getEyeHeight() * 0.8), (entityiterator.getZ() + z),
								(float) (entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).OldYAW,
								(float) (entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).OldPitch, 0, (float) 0.475, (float) 0.475, (float) 0.475,
								255 << 24 | 69 << 16 | 0 << 8 | 255);
						release();
					}
				}
			}
