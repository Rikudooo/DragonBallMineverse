...
public class StaminaOverlayProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			StaminaOverlayProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				StaminaOverlayProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (target(2)) {
			...
			var1 = (player.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).Stamina;
			var2 = (player.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).MaxStamina;
			renderRectangle(30, 33, (float) (30 + 71 * (var1 / var2)), 37, 1, 255 << 24 | 132 << 16 | 24 << 8 | 255);
			renderTexts((new java.text.DecimalFormat("##").format(var1) + "/" + new java.text.DecimalFormat("##").format(var2)), 32, 33, 0, 0, (float) 0.5, 255 << 24 | 222 << 16 | 0 << 8 | 255, 0);
			release();
		}
	}
}
