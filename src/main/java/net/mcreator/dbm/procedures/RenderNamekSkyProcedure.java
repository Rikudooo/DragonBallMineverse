...
public class RenderNamekSkyProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != RenderNamekSkyProcedure.amount || seed != RenderNamekSkyProcedure.seed) {
			RenderNamekSkyProcedure.amount = amount;
			RenderNamekSkyProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	private static boolean execute(@Nullable Event event, ResourceKey<Level> dimension) {
		if (dimension == null)
			return false;
		if (dimension == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("dbm:namek"))) {
			RenderSystem.defaultBlendFunc();
			RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "nameksky" + ".png")));
