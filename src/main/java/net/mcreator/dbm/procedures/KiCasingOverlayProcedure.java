...
public class KiCasingOverlayProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			KiCasingOverlayProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				KiCasingOverlayProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		if (target(2)) {
			...
			renderTexture(29, 23, 1, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
			release();
		}
	}
}
