...
public class TeleportKaioshinkaiProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		String CurrentPlanet = "";
		{
			...
			for (Entity entityiterator : _entfound) {
				if ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).StoryGUI == 70
						|| (entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).StoryGUI == 71) {
					if (world instanceof ServerLevel _currentLevel) {
						Entity _entity = entityiterator;
						ServerLevel _nextLevel = _currentLevel.getServer().getLevel(ResourceKey.create(Registries.DIMENSION, new ResourceLocation("dbm:kaioshinkai")));
