...
public class ShenronCalledProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (ShenronCalledProcedure.bufferBuilder == null || !ShenronCalledProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (ShenronCalledProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					ShenronCalledProcedure.mode = mode;
					ShenronCalledProcedure.format = format;
					ShenronCalledProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					ShenronCalledProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					ShenronCalledProcedure.mode = mode;
					ShenronCalledProcedure.format = format;
					ShenronCalledProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					ShenronCalledProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		ShenronCalledProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			ShenronCalledProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		...
		if (DbmModVariables.MapVariables.get(world).ShenronCalled || !world.getEntitiesOfClass(ShenronEntity.class, AABB.ofSize(new Vec3(x, y, z), 1000, 1000, 1000), e -> true).isEmpty()) {
			...
			if (!world.getEntitiesOfClass(DragonBallE1Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty()
				...
				if (target(2)) {
			...
			} else if (!(!world.getEntitiesOfClass(DragonBallE1Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty()
					&& !world.getEntitiesOfClass(DragonBallE2Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty()
					&& !world.getEntitiesOfClass(DragonBallE3Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty()
					&& !world.getEntitiesOfClass(DragonBallE4Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty()
					&& !world.getEntitiesOfClass(DragonBallE5Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty()
					&& !world.getEntitiesOfClass(DragonBallE6Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty()
					&& !world.getEntitiesOfClass(DragonBallE7Entity.class, AABB.ofSize(new Vec3(x, y, z), range, range, range), e -> true).isEmpty())
					&& !world.getEntitiesOfClass(ShenronEntity.class, AABB.ofSize(new Vec3(x, y, z), 1000, 1000, 1000), e -> true).isEmpty()) {
				if (begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR, false)) {
		...
		if (DbmModVariables.MapVariables.get(world).DragonBallsUsable == false) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator.getPersistentData().getDouble("dragonballScatter") > 0 && entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("dbm:earthdragonball")))) {
						if (begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR, false)) {
