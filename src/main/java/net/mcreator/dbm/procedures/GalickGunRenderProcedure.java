...
public class GalickGunRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (GalickGunRenderProcedure.bufferBuilder == null || !GalickGunRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (GalickGunRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					GalickGunRenderProcedure.mode = mode;
					GalickGunRenderProcedure.format = format;
					GalickGunRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					GalickGunRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					GalickGunRenderProcedure.mode = mode;
					GalickGunRenderProcedure.format = format;
					GalickGunRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					GalickGunRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		GalickGunRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			GalickGunRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
			...
			for (Entity entityiterator : _entfound) {
				if ((entityiterator instanceof StoryVegetaEntity _datEntS ? _datEntS.getEntityData().get(StoryVegetaEntity.DATA_SelectedKiAttack) : "").equals("Galick Gun")) {
					if (target(2)) {
						speed = 0.4;
						width = 0.5;
						length = entityiterator instanceof StoryVegetaEntity _datEntI ? _datEntI.getEntityData().get(StoryVegetaEntity.DATA_KiAttackSize) : 0;
						RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "kiattacks/galickgun_wave" + ".png")));
						if (begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX_COLOR, (!Minecraft.getInstance().isPaused()))) {
							i = width / (-2);
							j = width / 2;
						...
						renderShape(shape(), (entityiterator.getX() - Math.sin(Math.toRadians(entityiterator instanceof StoryVegetaEntity _datEntI ? _datEntI.getEntityData().get(StoryVegetaEntity.DATA_OldYAW) : 0)) * 1.5),
								...
								(float) (entityiterator instanceof StoryVegetaEntity _datEntI ? _datEntI.getEntityData().get(StoryVegetaEntity.DATA_OldPITCH) : 0), (float) (-1.6), (float) (-1.6), (float) (-1.6), 1,
								32 << 24 | 255 << 16 | 255 << 8 | 255);
						renderShape(shape(), (entityiterator.getX() - Math.sin(Math.toRadians(entityiterator instanceof StoryVegetaEntity _datEntI ? _datEntI.getEntityData().get(StoryVegetaEntity.DATA_OldYAW) : 0)) * 1.5),
								(entityiterator.getY() + entityiterator.getEyeHeight() * 0.9),
								(entityiterator.getZ() + Math.cos(Math.toRadians(entityiterator instanceof StoryVegetaEntity _datEntI ? _datEntI.getEntityData().get(StoryVegetaEntity.DATA_OldYAW) : 0)) * 1.5),
