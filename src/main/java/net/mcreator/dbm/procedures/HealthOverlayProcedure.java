...
public class HealthOverlayProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			HealthOverlayProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				HealthOverlayProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (target(2)) {
			...
			maximumhealth = (player.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).MaxHealth;
			renderRectangle(30, 15, (float) (30 + 100 * (health / maximumhealth)), 23, 1, 255 << 24 | 0 << 16 | 153 << 8 | 0);
			renderTexts((new java.text.DecimalFormat("##").format(health) + "/" + new java.text.DecimalFormat("##").format(maximumhealth)), 32, 15, 0, 0, 1, 255 << 24 | 19 << 16 | 255 << 8 | 3, 0);
			release();
		}
	}
}
