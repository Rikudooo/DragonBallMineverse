...
public class RenderStarsOnDragonBallsProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (RenderStarsOnDragonBallsProcedure.bufferBuilder == null || !RenderStarsOnDragonBallsProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (RenderStarsOnDragonBallsProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					RenderStarsOnDragonBallsProcedure.mode = mode;
					RenderStarsOnDragonBallsProcedure.format = format;
					RenderStarsOnDragonBallsProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					RenderStarsOnDragonBallsProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					RenderStarsOnDragonBallsProcedure.mode = mode;
					RenderStarsOnDragonBallsProcedure.format = format;
					RenderStarsOnDragonBallsProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					RenderStarsOnDragonBallsProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		RenderStarsOnDragonBallsProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			RenderStarsOnDragonBallsProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
