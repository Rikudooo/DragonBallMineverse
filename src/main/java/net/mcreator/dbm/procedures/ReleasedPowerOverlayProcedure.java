...
public class ReleasedPowerOverlayProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			ReleasedPowerOverlayProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				ReleasedPowerOverlayProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (target(2)) {
			...
			var1 = (player.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).ReleasedPower;
			renderRectangle(30, 31, (float) (30 + 100 * var1), 32, 0, 255 << 24 | 0 << 16 | 213 << 8 | 135);
			release();
		}
	}
}
