...
public class TribeamRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (TribeamRenderProcedure.bufferBuilder == null || !TribeamRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (TribeamRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					TribeamRenderProcedure.mode = mode;
					TribeamRenderProcedure.format = format;
					TribeamRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					TribeamRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					TribeamRenderProcedure.mode = mode;
					TribeamRenderProcedure.format = format;
					TribeamRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					TribeamRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		TribeamRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			TribeamRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
