...
public class ShenronThereProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (ShenronThereProcedure.bufferBuilder == null || !ShenronThereProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (ShenronThereProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					ShenronThereProcedure.mode = mode;
					ShenronThereProcedure.format = format;
					ShenronThereProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					ShenronThereProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					ShenronThereProcedure.mode = mode;
					ShenronThereProcedure.format = format;
					ShenronThereProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					ShenronThereProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		ShenronThereProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			ShenronThereProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
