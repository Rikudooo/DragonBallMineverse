...
public class ShenronThereProcedure {
	...
	private static final Predicate<Object[]> PREDICATE = params -> {
		...
		if (entity != null) {
			...
			execute(null, level, pos.x(), pos.y(), pos.z(), entity);
			return true;
		}
		return false;
	};
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != ShenronThereProcedure.amount || seed != ShenronThereProcedure.seed) {
			ShenronThereProcedure.amount = amount;
			ShenronThereProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((entity.level().dimension()) == Level.OVERWORLD) {
			if (!world.getEntitiesOfClass(ShenronEntity.class, AABB.ofSize(new Vec3(x, y, z), 1000, 1000, 1000), e -> true).isEmpty()) {
