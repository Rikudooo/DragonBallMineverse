...
public class SphereOfDestructionRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (SphereOfDestructionRenderProcedure.bufferBuilder == null || !SphereOfDestructionRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (SphereOfDestructionRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					SphereOfDestructionRenderProcedure.mode = mode;
					SphereOfDestructionRenderProcedure.format = format;
					SphereOfDestructionRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					SphereOfDestructionRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					SphereOfDestructionRenderProcedure.mode = mode;
					SphereOfDestructionRenderProcedure.format = format;
					SphereOfDestructionRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					SphereOfDestructionRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		SphereOfDestructionRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			SphereOfDestructionRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		...
		double scale2 = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof SphereOfDestructionEntity) {
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, (float) 1.6, (float) 1.6, (float) 1.6, 180 << 24 | 235 << 16 | 101 << 8 | 255);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, 2, 2, 2, 255 << 24 | 230 << 16 | 185 << 8 | 255);
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, (float) 2.5, (float) 2.5, (float) 2.5, 100 << 24 | 190 << 16 | 80 << 8 | 255);
						release();
					}
				}
			}
