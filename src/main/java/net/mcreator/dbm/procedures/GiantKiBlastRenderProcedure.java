...
public class GiantKiBlastRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (GiantKiBlastRenderProcedure.bufferBuilder == null || !GiantKiBlastRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (GiantKiBlastRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					GiantKiBlastRenderProcedure.mode = mode;
					GiantKiBlastRenderProcedure.format = format;
					GiantKiBlastRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					GiantKiBlastRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					GiantKiBlastRenderProcedure.mode = mode;
					GiantKiBlastRenderProcedure.format = format;
					GiantKiBlastRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					GiantKiBlastRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		GiantKiBlastRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			GiantKiBlastRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		...
		double scale2 = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof GiantKiBlastEntity) {
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, (float) 2.5, (float) 2.5, (float) 2.5, 255 << 24 | 134 << 16 | 217 << 8 | 255);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, (float) 2.6, (float) 2.6, (float) 2.6, 255 << 24 | 28 << 16 | 160 << 8 | 255);
						release();
					}
				}
			}
