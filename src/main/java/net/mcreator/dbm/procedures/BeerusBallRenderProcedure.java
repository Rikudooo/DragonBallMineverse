...
public class BeerusBallRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (BeerusBallRenderProcedure.bufferBuilder == null || !BeerusBallRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (BeerusBallRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					BeerusBallRenderProcedure.mode = mode;
					BeerusBallRenderProcedure.format = format;
					BeerusBallRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					BeerusBallRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					BeerusBallRenderProcedure.mode = mode;
					BeerusBallRenderProcedure.format = format;
					BeerusBallRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					BeerusBallRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		BeerusBallRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			BeerusBallRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		...
		double scale2 = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof BeerusBallEntity) {
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, 4, 4, 4, 255 << 24 | 255 << 16 | 220 << 8 | 175);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, (float) 9.5, (float) 9.5, (float) 9.5, 255 << 24 | 255 << 16 | 70 << 8 | 0);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, 10, 10, 10, 255 << 24 | 255 << 16 | 244 << 8 | 205);
						release();
					}
				}
			}
