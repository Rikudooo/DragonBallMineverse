...
public class HealthCasingOverlayProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			HealthCasingOverlayProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				HealthCasingOverlayProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		if (target(2)) {
			RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + "barcasing" + ".png")));
			renderTexture(29, 14, 1, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
			release();
		}
	}
}
