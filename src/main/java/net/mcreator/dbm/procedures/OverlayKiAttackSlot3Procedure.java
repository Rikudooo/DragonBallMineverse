...
public class OverlayKiAttackSlot3Procedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			OverlayKiAttackSlot3Procedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				OverlayKiAttackSlot3Procedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		if (target(2)) {
			...
			Slot = (player.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiSlot3;
			RenderSystem.setShaderTexture(0, new ResourceLocation(("dbm" + ":textures/" + ("kiattacks/" + (Slot.replace(" ", "")).toLowerCase()) + ".png")));
			renderTexture((float) (Minecraft.getInstance().getWindow().getGuiScaledWidth() - 112.5), (float) (Minecraft.getInstance().getWindow().getGuiScaledHeight() - 14), 0, 0, (float) 0.5, 255 << 24 | 255 << 16 | 255 << 8 | 255, 8);
			release();
		}
	}
}
