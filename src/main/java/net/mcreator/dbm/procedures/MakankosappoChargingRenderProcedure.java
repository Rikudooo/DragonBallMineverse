...
public class MakankosappoChargingRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (MakankosappoChargingRenderProcedure.bufferBuilder == null || !MakankosappoChargingRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (MakankosappoChargingRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					MakankosappoChargingRenderProcedure.mode = mode;
					MakankosappoChargingRenderProcedure.format = format;
					MakankosappoChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					MakankosappoChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					MakankosappoChargingRenderProcedure.mode = mode;
					MakankosappoChargingRenderProcedure.format = format;
					MakankosappoChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					MakankosappoChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		MakankosappoChargingRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			MakankosappoChargingRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
