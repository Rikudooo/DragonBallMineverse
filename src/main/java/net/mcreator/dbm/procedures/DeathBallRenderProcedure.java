...
public class DeathBallRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (DeathBallRenderProcedure.bufferBuilder == null || !DeathBallRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (DeathBallRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					DeathBallRenderProcedure.mode = mode;
					DeathBallRenderProcedure.format = format;
					DeathBallRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					DeathBallRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					DeathBallRenderProcedure.mode = mode;
					DeathBallRenderProcedure.format = format;
					DeathBallRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					DeathBallRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		DeathBallRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			DeathBallRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z) {
		...
		double scale2 = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof DeathBallEntity) {
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, (float) 2.5, (float) 2.5, (float) 2.5, 255 << 24 | 0 << 16 | 0 << 8 | 0);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX()), (entityiterator.getY() + entityiterator.getBbHeight() / 2), (entityiterator.getZ()), 0, 0, 0, 4, 4, 4, 100 << 24 | 120 << 16 | 0 << 8 | 255);
						release();
					}
				}
			}
