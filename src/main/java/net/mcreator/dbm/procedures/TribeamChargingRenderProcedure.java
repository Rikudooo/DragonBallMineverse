...
public class TribeamChargingRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (TribeamChargingRenderProcedure.bufferBuilder == null || !TribeamChargingRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (TribeamChargingRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					TribeamChargingRenderProcedure.mode = mode;
					TribeamChargingRenderProcedure.format = format;
					TribeamChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					TribeamChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					TribeamChargingRenderProcedure.mode = mode;
					TribeamChargingRenderProcedure.format = format;
					TribeamChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					TribeamChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		TribeamChargingRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			TribeamChargingRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
		...
		}
		{
			...
			for (Entity entityiterator : _entfound) {
				if ((entityiterator instanceof StoryTienshinhanEntity _datEntS ? _datEntS.getEntityData().get(StoryTienshinhanEntity.DATA_SelectedKiAttack) : "").equals("Tribeam")) {
					xsin = Math.sin(Math.toRadians(entityiterator instanceof StoryTienshinhanEntity _datEntI ? _datEntI.getEntityData().get(StoryTienshinhanEntity.DATA_OldYAW) : 0)) * (-4);
					xcos = Math.cos(Math.toRadians(entityiterator instanceof StoryTienshinhanEntity _datEntI ? _datEntI.getEntityData().get(StoryTienshinhanEntity.DATA_OldYAW) : 0)) * 4;
					if (begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR, false)) {
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX() + xsin), (entityiterator.getY() + entityiterator.getEyeHeight() * 0.9), (entityiterator.getZ() + xcos),
								...
								(float) (entityiterator instanceof StoryTienshinhanEntity _datEntI ? _datEntI.getEntityData().get(StoryTienshinhanEntity.DATA_OldPITCH) : 0), 0, (float) 4.5, (float) 4.5, (float) 4.5,
								255 << 24 | 255 << 16 | 255 << 8 | 185);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX() + xsin), (entityiterator.getY() + entityiterator.getEyeHeight() * 0.9), (entityiterator.getZ() + xcos),
								(float) (entityiterator instanceof StoryTienshinhanEntity _datEntI ? _datEntI.getEntityData().get(StoryTienshinhanEntity.DATA_OldYAW) : 0),
								(float) (entityiterator instanceof StoryTienshinhanEntity _datEntI ? _datEntI.getEntityData().get(StoryTienshinhanEntity.DATA_OldPITCH) : 0), 0, (float) 4.6, (float) 4.6, (float) 4.6,
