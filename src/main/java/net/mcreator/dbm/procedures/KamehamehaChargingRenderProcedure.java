...
public class KamehamehaChargingRenderProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (KamehamehaChargingRenderProcedure.bufferBuilder == null || !KamehamehaChargingRenderProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (KamehamehaChargingRenderProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					KamehamehaChargingRenderProcedure.mode = mode;
					KamehamehaChargingRenderProcedure.format = format;
					KamehamehaChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KamehamehaChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					KamehamehaChargingRenderProcedure.mode = mode;
					KamehamehaChargingRenderProcedure.format = format;
					KamehamehaChargingRenderProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					KamehamehaChargingRenderProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		KamehamehaChargingRenderProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			KamehamehaChargingRenderProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
		...
		for (Entity entityiterator : new ArrayList<>(world.players())) {
			if ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).ChargingKiAttack == true) {
				if (((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).SelectedKiAttack).equals("Kamehameha")) {
					x = Math.sin(Math.toRadians(entityiterator.getLookAngle().x)) * (-1);
					z = Math.cos(Math.toRadians(entityiterator.getLookAngle().z)) * 1.2;
					...
					if (target(2)) {
						renderShape(shape(), (entityiterator.getX() + x), (entityiterator.getY() + entityiterator.getBbHeight() * 0.55), (entityiterator.getZ() + z), 0, 0, 0,
								(float) (0.8 * ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiAttackCharge / 200)),
								(float) (0.8 * ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiAttackCharge / 200)),
								(float) (0.8 * ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiAttackCharge / 200)), 255 << 24 | 244 << 16 | 253 << 8 | 253);
						RenderSystem.blendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
						renderShape(shape(), (entityiterator.getX() + x), (entityiterator.getY() + entityiterator.getBbHeight() * 0.55), (entityiterator.getZ() + z), 0, 0, 0,
								(float) (1 * ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiAttackCharge / 200)),
								(float) (1 * ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiAttackCharge / 200)),
								(float) (1 * ((entityiterator.getCapability(DbmModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new DbmModVariables.PlayerVariables())).KiAttackCharge / 200)), 255 << 24 | 134 << 16 | 217 << 8 | 255);
						release();
					}
				}
			}
