...
public class DBMAdminCommand {
	...
	public static void registerCommand(RegisterCommandsEvent event) {
		event.getDispatcher().register(Commands.literal("dbmadmin").requires(s -> s.hasPermission(4)).then(
				Commands.literal("stats").then(Commands.literal("tp").then(Commands.argument("Player", EntityArgument.player()).then(Commands.literal("add").then(Commands.argument("amount", DoubleArgumentType.doubleArg()).executes(arguments -> {
